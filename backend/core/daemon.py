...
def background_sync():
            ...
            def run_google(a=acc):
                try:
                    ...
                    return {
                        "events": [],
                        "calendars": [],
                        "error": f"Google Account {a.get('name')}: offline ({e!s})"
                        if is_off
                        else f"Google Account {a.get('name')}: {e!s}",
                        "offline": is_off,
                        "account_id": a["id"],
                    }
            ...
            def run_nc(a=acc):
                try:
                    ...
                    return {
                        "events": [],
                        "calendars": [],
                        "error": f"Nextcloud {a.get('name')}: offline ({e!s})"
                        if is_off
                        else f"Nextcloud {a.get('name')}: {e!s}",
                        "offline": is_off,
                        "account_id": a["id"],
                    }
            ...
            def run_ic(a=acc):
                try:
                    ...
                    return {
                        "events": [],
                        "calendars": [],
                        "error": f"iCloud {a.get('name')}: offline ({e!s})"
                        if is_off
                        else f"iCloud {a.get('name')}: {e!s}",
                        "offline": is_off,
                        "account_id": a["id"],
                    }
        ...
        def run_ics():
            try:
                ...
                return {
                    "events": [],
                    "calendars": [],
                    "error": f"ICS Subscriptions: offline ({e!s})" if is_off else f"ICS Subscriptions: {e!s}",
                    "offline": is_off,
                    "account_id": "ics",
                }
    ...
    output = {"events": merged_events, "calendars": list(merged_calendars.values())}
    if errors:
