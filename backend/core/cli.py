...
def select_account_index(accounts):
    ...
    for i, acc in enumerate(accounts):
        print(
            f"  [{i + 1}] {acc.get('name')} ({acc.get('email', acc.get('username', acc.get('url', acc.get('path', ''))))})"
        )
...
def handle_toggle_account_cli(args):
    if len(args) < 5:
        print(
            "Error: Missing arguments for toggle-account. Usage: waylandar toggle-account <provider> <account_id> <true/false>",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if not found:
        print(
            f"Error: Account/Source '{account_id}' not found under provider '{provider}'",
            file=sys.stderr,
        )
        sys.exit(1)
...
def setup_vdirsyncer(config):
    ...
    try:
        ...
        directories.append({"path": path, "name": name, "enabled": True})
        save_config(config)
